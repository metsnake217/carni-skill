...
alexa.response = function() {
  ...
  this.say = function(str) {
     console.log("gnagna response saying " + str);
...
alexa.app = function(name, endpoint) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      response.send = function(exception) {
        if (typeof self.post == "function" && !postExecuted) {
          postExecuted = true;
          console.log("gnagna sending response");

    console.log("gnagna req= " + request.param("location") + " - resp= " + response);
    console.log("gnagna = endpoint= " + self.endpoint);
    /*express.get(path, function(req, res) {
      console.log("gnagna = get endpoint: " + self.endpoint);*/
      res.render("index", {
        title : 'results',
        matches : null,
        loggedIn : true,
    ...
    console.log("gnagna restarted");
      ...
      try {
        ...
        if (typeof self.pre == "function") {
          console.log("gnagna sending self pre");
          self.pre(request, response, requestType);
        }
        if (!response.resolved) {
          //console.log("gnagna: response not resolved: " + requestType);
          if ("IntentRequest" === requestType) {
            console.log("gnagna sending intent request");
            ...
            if (typeof self.intents[intent] != "undefined" && typeof self.intents[intent]["function"] == "function") {
              if (false !== self.intents[intent]["function"](request, response)) {
                console.log("gnagna sending self intent");
                response.send();
              }
            } else {
            ...
            //Next three lines added by gnagna
            //console.log("gnagna0: response not resolved SESSION ENDED: " + requestType);
            response.say("Our conversation is coming to an end. Goodbye!");
            response.send();
            if (typeof self.sessionEndedFunc == "function") {
              //console.log("gnagna1: response not resolved SESSION ENDED: " + requestType);
              if (false !== self.sessionEndedFunc(request, response)) {
                //console.log("gnagna2: " + requestType);
                response.send();
              }
            }
          } else {
  ...
  */
  ...
  // A utility method to bootstrap alexa endpoints into express automatically
  this.express = function(express, path, location, time) {
    console.log("gnagna path= " + path + " - loc= " + location + " - time= " + time);
    console.log("gnagna = endpoint= " + self);
    express.get(path, function(req, res) {
      console.log("gnagna = get endpoint: " + self.endpoint);
      res.render(path, {
        title : 'results',
